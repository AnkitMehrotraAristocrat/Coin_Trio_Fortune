...
namespace PixelUnited.NMG.Slots.Milan.GAMEID {
	/// <summary>
	/// A state machine presenter that handles free spins outro presentation.
	/// </summary>
	public class GenericOutroPresenter : MonoBehaviour, IStatePresenter, ServiceLocator.IHandler {
		[Preserve]
		[Serializable]
		...
		[SerializeField] private float _outroWait = 8.0F;
		[SerializeField] private TextMeshPro _winAmountText;
		[SerializeField] private bool _showWin = false;
		...
		public IEnumerator<Yield> Enter() {
			...
			if (_showWin) {
				_winAmountText.text = NumberFormat.CommaDelimited(_winAmountProvider.WinAmount);
			}
		...
		private IEnumerator<Yield> StartOutroTimeout() {
			yield return new YieldForSeconds(_outroWait);
		}
